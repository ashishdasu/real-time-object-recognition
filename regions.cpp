...
void segmentRegions(const cv::Mat &binary,
    ...
    int nLabels = cv::connectedComponentsWithStats(binary, regionMap, stats, centroids);
    ...
    // Keep the largest regions — smaller ones are likely debris or partial objects
    std::sort(regions.begin(), regions.end(),
              [](const RegionInfo &a, const RegionInfo &b) { return a.area > b.area; });

    if ((int)regions.size() > MAX_REGIONS)
